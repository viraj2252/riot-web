...
const REPOS = ['vector-im/vector-web', 'matrix-org/matrix-react-sdk-vj', 'matrix-org/matrix-js-sdk'];
...
export default class ChangelogDialog extends React.Component {
        ...
        _elementsForCommit(commit) {
            return ( <
                ...
                target = "_blank"
                rel = "noopener" > { commit.commit.message } <
                /a> <
                /li>
            );
        }
        ...
        render() {
            ...
            const logs = REPOS.map(repo => {
                ...
                return ( <
                    ...
                    <
                    h2 > { repo } < /h2> <
                    ul > { this.state[repo].map(this._elementsForCommit) } <
                    /ul> <
                    /div>
                )
            });
            ...
            const content = ( <
                ...
                return ( <
                    QuestionDialog title = { _t("Changelog") }
                    description = { content }
                    button = { _t("Update") }
